...
# ==============================
# ANLI-CI LAUNCHER + INTI (Versi Bahasa Indonesia)
...
# ==============================
# === ANLI-CI INTI (v4.10.3) ===
# ==============================

version_number="Versi 4.10.3"
...
help_info() {
    ...
    Options:
      -c, --continue
        Lanjutkan menonton dari riwayat
      -d, --download
        Unduh video alih-alih memutar
      -D, --delete
        Hapus riwayat
      -l, --logview
        Tampilkan log
      -s, --syncplay
        Tonton bersama teman via Syncplay
      -S, --select-nth
        Pilih entri ke-n
      -q, --quality
        Tentukan kualitas video
      -m, --mpv
        Gunakan MPV untuk memutar
      -V, --version
        Tampilkan versi skrip
      -h, --help
        Tampilkan pesan bantuan ini
      -e, --episode, -r, --range
        Tentukan rentang episode
      --dub
        Putar versi dub (bahasa lain)
      --rofi
        Gunakan rofi alih-alih fzf
      --skip
        Lewati intro (hanya mpv)
      --no-detach
        Jangan jalankan player di latar
      --exit-after-play
        Keluar setelah selesai memutar
      --skip-title <title>
        Judul untuk anli-skip
      -N, --nextep-countdown
        Tampilkan hitung mundur episode berikutnya
      -U, --update
        Perbarui skrip ini
    Contoh penggunaan:
      ...
      %s --mpv cyberpunk edgerunners -q 1080p -e 4
      %s blue lock -e 5-6
      %s -e \"5 6\" blue lock
    \n" "${0##*/}" "${0##*/}" "${0##*/}" "${0##*/}" "${0##*/}" "${0##*/}" "${0##*/}" "${0##*/}" "${0##*/}"
    exit 0
...
update_script() {
    ...
    update="$(printf '%s\n' "$update" | diff -u "$0" -)"
    if [ -z "$update" ]; then
        printf "Skrip sudah terbaru :)\n"
    else
        if printf '%s\n' "$update" | patch "$0" -; then
            printf "Skrip selesai di update\n"
        else
            die "Gagal mengupdate skrip!"
			die "periksa koneksi internet anda,dan coba lagi..."
        fi
    fi
    exit 0
}
...
dep_ch() {
    for dep; do
        command -v "${dep%% *}" >/dev/null || die "Program \"${dep%% *}\" Tidak di temukan.Mohon install terlebih dahulu."
    done
}
...
| >|;/EXT-X-I-FRAME/d' | sed "s|>|cc>${relative_link}|g" | sort -nr
    ...
    printf "\033[1;32m%s\033[0m Link berhasil di ambil\n" "$provider_name" 1>&2
}
...
select_quality() {
    ...
    [ -z "$result" ] && printf "Kualitas tidak di temukan,mengambil kualitas terbaik\n" 1>&2 && result=$(printf "%s" "$links" | head -n1)
...
get_episode_url() {
    ...
    if printf "%s" "$ep_list" | grep -q "^$ep_no$"; then
        [ -z "$episode" ] && die "Episode di temukan,dan sudah rilis,tetapi tidak ada sumber yang valid!"
    else
...
play_episode() {
    [ "$log_episode" = 1 ] && [ "$player_function" != "debug" ] && [ "$player_function" != "download" ] && command -v logger >/dev/null && logger -t anli-ci.sh "${allanime_title}${ep_no}"
...
mode="${ANLI_CI_MODE:-sub}"
download_dir="${ANLI_CI_DOWNLOAD_DIR:-.}"
log_episode="${ANLI_CI_LOG:-1}"
quality="${ANLI_CI_QUALITY:-best}"
case "$(uname -a | cut -d " " -f 1,3-)" in
    *Darwin*) player_function="${ANLI_CI_PLAYER:-$(where_iina)}" ;;   # mac OS
    *ndroid*) player_function="${ANLI_CI_PLAYER:-android_vlc}" ;;     # Android OS (termux)
    *MINGW* | *WSL2*) player_function="${ANLI_CI_PLAYER:-mpv.exe}" ;; # Windows OS
    *ish*) player_function="${ANLI_CI_PLAYER:-iSH}" ;;                # iOS (iSH)
    *) player_function="${ANLI_CI_PLAYER:-$(where_mpv)}" ;;           # Linux OS
esac

no_detach="${ANLI_CI_NO_DETACH:-0}"
exit_after_play="${ANLI_CI_EXIT_AFTER_PLAY:-0}"
use_external_menu="${ANLI_CI_EXTERNAL_MENU:-0}"
external_menu_normal_window="${ANLI_CI_EXTERNAL_MENU_NORMAL_WINDOW:-0}"
skip_intro="${ANLI_CI_SKIP_INTRO:-0}"
# shellcheck disable=SC2154
skip_title="$ANLI_CI_SKIP_TITLE"
[ -t 0 ] || use_external_menu=1
hist_dir="${ANLI_CI_HIST_DIR:-${XDG_STATE_HOME:-$HOME/.local/state}/anli-ci.sh}"
[ ! -d "$hist_dir" ] && mkdir -p "$hist_dir"
histfile="$hist_dir/ani-hsts"
[ ! -f "$histfile" ] && : >"$histfile"
search="${ANLI_CI_DEFAULT_SOURCE:-scrape}"
    ...
    case "$1" in
        ...
        -l | --logview)
            case "$(uname -s)" in
                ...
                Linux*) journalctl -t anli-ci.sh ;;
                *) die "gagal mengimplementasikan fitur ini di perangkat anda" ;;
            esac
            exit 0
            ;;
...
[ "$use_external_menu" = "0" ] && multi_selection_flag="${ANLI_CI_MULTI_SELECTION:-"-m"}"
[ "$use_external_menu" = "1" ] && multi_selection_flag="${ANLI_CI_MULTI_SELECTION:-"-multi-select"}"
[ "$external_menu_normal_window" = "1" ] && external_menu_args="-normal-window"
printf "\33[2K\r\033[1;34mMemeriksa depedensi...\033[0m\n"
...
case "$player_function" in
    ...
    android*) printf "\33[2K\rMemeriksa player di android di matikan\n" ;;
    *iSH*) printf "\33[2K\rMemeriksa player di ios di matikan\n" ;;
